...
if (isset($_GET['toggle_featured']) && is_numeric($_GET['toggle_featured'])) {
    ...
    mysqli_query($conn, "UPDATE products SET is_featured = 1 - is_featured WHERE id=$tf_id");
...
<div class="p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            ...
            <p class="text-gray-500 text-sm"><?= $total ?> produk ditemukan</p>
        </div>
        <a href="<?= BASE_URL ?>/admin/add_product.php"
           class="bg-maroon-800 hover:bg-maroon-700 text-white font-bold px-5 py-2.5 rounded-xl flex items-center space-x-2 transition-colors">
            <i class="fas fa-plus text-sm"></i><span>Tambah Produk</span>
        </a>
    </div>
    ...
    <form method="GET" class="mb-5">
        <div class="relative max-w-sm">
            <input type="text" name="search" value="<?= htmlspecialchars($search) ?>"
                   placeholder="Cari nama, era, kategori..."
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-maroon-800 text-white text-left">
                        ...
                        <th class="px-4 py-3 font-semibold text-center hidden sm:table-cell">Stok</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr><td colspan="7" class="text-center py-12 text-gray-400">
                        <i class="fas fa-box-open text-4xl block mb-3 text-gray-200"></i>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <div class="flex items-center space-x-3">
                                <div class="w-12 h-12 bg-cream-200 rounded-xl overflow-hidden shrink-0 flex items-center justify-center">
                                    <?php if ($p['image'] && file_exists(__DIR__.'/../uploads/'.$p['image'])): ?>
                                        <img src="<?= BASE_URL ?>/uploads/<?= htmlspecialchars($p['image']) ?>" class="w-full h-full object-cover">
                                    <?php else: ?>
                                        <i class="fas fa-gem text-maroon-300 text-sm"></i>
                                    <?php endif; ?>
                                </div>
                                <div>
                        ...
                        <td class="px-4 py-3 hidden lg:table-cell">
                            <span class="text-xs px-2 py-0.5 rounded-full font-bold <?= match($p['condition_status']) {
                                'Excellent' =>'bg-green-100 text-green-700',
                                'Very Good' =>'bg-blue-100 text-blue-700',
                                'Good'      =>'bg-yellow-100 text-yellow-700',
                                default     =>'bg-red-100 text-red-700'
                            } ?>"><?= htmlspecialchars($p['condition_status']) ?></span>
                        </td>
                        <td class="px-4 py-3 text-center hidden sm:table-cell">
                            <span class="font-bold <?= $p['stock']>0?'text-green-600':'text-red-500' ?>"><?= $p['stock'] ?></span>
                        </td>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center space-x-1.5">
                                ...
                                <a href="<?= BASE_URL ?>/admin/edit_product.php?id=<?= $p['id'] ?>"
                                   title="Edit produk"
                                ...
                                <a href="?delete=<?= $p['id'] ?><?= $search?'&search='.urlencode($search):'' ?>"
                                   title="Hapus produk"
                                   onclick="return confirm('Hapus produk \'<?= addslashes(htmlspecialchars($p['name'])) ?>\'?')"
        ...
        <div class="flex justify-between items-center px-5 py-4 border-t border-gray-100 bg-gray-50">
            <p class="text-xs text-gray-500">Halaman <?= $pg ?> dari <?= $total_pages ?> (<?= $total ?> produk)</p>
            <div class="flex space-x-1">
                <?php if ($pg > 1): ?>
                <a href="?pg=<?= $pg-1 ?><?= $search?'&search='.urlencode($search):'' ?>"
                   class="w-8 h-8 rounded-lg bg-white border border-gray-200 flex items-center justify-center hover:bg-maroon-50 text-gray-600 transition-colors">
                ...
                <a href="?pg=<?= $i ?><?= $search?'&search='.urlencode($search):'' ?>"
                   class="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold transition-colors
                          <?= $i===$pg?'bg-maroon-800 text-white':'bg-white border border-gray-200 text-gray-600 hover:bg-maroon-50' ?>">
                    <?= $i ?>
                </a>
                <?php endfor; ?>
